...
def main():
    ...
    while True:
        ...
        if len(edge_list) < 4:
            ...
            dst = pers.trans_pers(src)
            adjusted_color = pers.trans_pers(adjusted_color)
            adjusted_threshold = pers.trans_pers(adjusted_threshold)
        ...
        if ch == ord('q'):
            ...
            next_arg = pos_to_arg(next_pos)
            reverse_arg = pos_to_arg(reverse_pos)
            send.sendnum(next_arg)
            send.sendnum(reverse_arg)
            print next_pos
            print reverse_pos
